...
def cal_20year_difference(data, varname):
    #print(data)
    lat       = data.lat.data
    lon       = data.lon.data
...
def bootstrap(data):
    ...
    res  = bootstrap(data, np.average, batch=8, n_resamples=9999, confidence_level=0.95, random_state=rng)
...
def plot_ecdf(data,labelx,labely,title,color,out_path,out_name,low,high,adjustment):
    """Plot ecdf"""
...
def main():
    ...
    out_path = "/exports/csce/datastore/geos/users/s2618078/data/model_data/B1850/"
    ...
    sample_prect = cal_20year_difference(f0_PRECC, 'PRECC') + cal_20year_difference(f0_PRECL, 'PRECL')
    ...
    plot_ecdf(result.bootstrap_distribution," ", "Cumulative probability (%) ", "Empirical CDF", "r", path_src, "Aerosol_Research_CESM_PRECT_40years_trend_Bootstrap_CDF.pdf", result.confidence_interval[0], result.confidence_interval[1], 0.03)
