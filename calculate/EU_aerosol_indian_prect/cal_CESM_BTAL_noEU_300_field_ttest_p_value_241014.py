...
def calculation():
    ...
    f_z_BTAL = xr.open_dataset(path_src + file_z_BTAL).sel(lev=300)
    f_v_BTAL = xr.open_dataset(path_src + file_v_BTAL).sel(lev=300)
    f_w_BTAL = xr.open_dataset(path_src + file_w_BTAL).sel(lev=500)
    f_u_BTAL = xr.open_dataset(path_src + file_u_BTAL).sel(lev=300)
    f_z_BTALnEU = xr.open_dataset(path_src + file_z_BTALnEU).sel(lev=300)
    f_v_BTALnEU = xr.open_dataset(path_src + file_v_BTALnEU).sel(lev=300)
    f_w_BTALnEU = xr.open_dataset(path_src + file_w_BTALnEU).sel(lev=500)
    f_u_BTALnEU = xr.open_dataset(path_src + file_u_BTALnEU).sel(lev=300)
    ...
    ncfile.to_netcdf(out_path + 'EUI_CESM_BTAL_BTALnEU_high_level_but_single_level_300_ensemble_mean_UVWZ_DIV_and_ttest.nc')
