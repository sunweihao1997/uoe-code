...
def calculation():
    ...
    f_z_BTAL = xr.open_dataset(path_src + file_z_BTAL).sel(lev=200)
    f_v_BTAL = xr.open_dataset(path_src + file_v_BTAL).sel(lev=200)
    f_w_BTAL = xr.open_dataset(path_src + file_w_BTAL).sel(lev=500)
    f_u_BTAL = xr.open_dataset(path_src + file_u_BTAL).sel(lev=200)
    f_z_BTALnEU = xr.open_dataset(path_src + file_z_BTALnEU).sel(lev=200)
    f_v_BTALnEU = xr.open_dataset(path_src + file_v_BTALnEU).sel(lev=200)
    f_w_BTALnEU = xr.open_dataset(path_src + file_w_BTALnEU).sel(lev=500)
    f_u_BTALnEU = xr.open_dataset(path_src + file_u_BTALnEU).sel(lev=200)
    ...
    ncfile.attrs['description'] = 'UVZ and DIVERGENCEat 200 hPa, ttest for the period 1945-1960. Created by /home/sun/uoe-code/calculate/EU_aerosol_indian_prect/cal_CESM_BTAL_noEU_200_field_ttest_p_value_1940_1960_v2_241217.py.'
    # Save all of them to the netcdf
    out_path = '/home/sun/data/download_data/data/analysis_data/analysis_EU_aerosol_climate_effect/'
    ncfile.to_netcdf(out_path + 'EUI_CESM_BTAL_BTALnEU_high_level_but_single_level_200_ensemble_mean_UVWZ_DIV_and_ttest_1945_1960_JJA.nc')
