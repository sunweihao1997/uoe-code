...
#    b, a = scipy.signal.butter(N, Wn, 'lowpass')
    ...
    for j in range(num_sample):
        ...
        slope, intercept, r_value, p_value, std_err = scipy.stats.linregress(np.linspace(1, 55, 55), JJAS_PRECT[j:j+55])
        sample[j]  = slope * 10 # decade
...
def bootstrap(data):
    ...
    res  = bootstrap(data, np.average, batch=10, n_resamples=99999, confidence_level=0.99, random_state=rng)
