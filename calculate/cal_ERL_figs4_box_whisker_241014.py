...
def cal_single_JJA():
    ...
    #a = cal_JJA_average(test_file, "lat", "lon", lat1, lat2, lon1, lon2, np.array([6, 7, 8]), "PRECC")
    #print(a*86400000)
    ...
    for i in range(8):
        ...
        a = cal_JJA_average(f_precc, "lat", "lon", lat1, lat2, lon1, lon2, np.array([6, 7, 8]), "PRECC")

        slope_gpcc, intercept = np.polyfit(np.linspace(1, 60, 60), a[51:111], 1)

        btal_sample[i] = slope_gpcc
    
    #print(np.average(btal_sample*86400000*50))
    ...
    #print(btalneu_sample*86400000*50)
...
def main():
    ...
    # 计算四分位数
    Q1 = np.percentile(bootstrap_results_gpcc, 25) * 50
    Q3 = np.percentile(bootstrap_results_gpcc, 75) * 50
    ...
    ax.scatter(np.ones(len(btalneu_single_member))*2, btalneu_single_member * 50, zorder=10, marker='x', color='green')
    ax.scatter([2], np.average(btalneu_single_member*50), zorder=10, marker='o', color='red')
...
#    ax.boxplot(bootstrap_results_gpcc*50,)
...
#    ax.set_title('Box plot with custom whiskers')
    ...
    plt.savefig('/home/sun/paint/ERL/ERL_figs4.pdf')
