...
for i in range(len(lat)):
    for j in range(len(lon)):
        ...
        if np.count_nonzero(~np.isnan(data_01to55['PRECT_JJA_BTAL'].data[:, i, j])) > 1:
            slope, intercept, lower, upper = theilslopes(data_01to55['PRECT_JJA_BTAL'].data[:, i, j], np.linspace(1, 55, 55))
            jja_trend[i, j] = slope
            mk_result = mk.original_test(data_01to55['PRECT_JJA_BTAL'].data[:, i, j])
