...
test: test if the result calculated by windspharm is correct
'''
import xarray as xr
import numpy as np
import os
...
btal_div    =  np.average(data_p1['btal_div'], axis=0) - np.average(data_p2['btal_div'], axis=0)
btalneu_div =  np.average(data_p1['btalneu_div'], axis=0) - np.average(data_p2['btalneu_div'], axis=0)
