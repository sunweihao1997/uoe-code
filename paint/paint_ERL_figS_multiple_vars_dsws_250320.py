...
def main():
    ...
    level2 = np.array([-10, -8, -6, -4, -3, -2, -1.5, -1, -0.5, 0.5, 1, 1.5, 2, 3, 4, 6, 8, 10])
    print(level2.shape)
...
#    plot_diff_slp_wind(diff_slp=data_file["psl_btal_btalneu_diff"],    diff_u=data_file["u_btal_btalneu_diff"], diff_v=data_file["v_btal_btalneu_diff"] , left_title='(a)', right_title='BTAL - BTALnEU', out_path=out_path, pic_name="Aerosol_research_ERL_2a_BTAL_BTALnEU.pdf", p=data_file['psl_btal_btalneu_diffp'], level=level2)
    plot_diff_slp_wind(diff_slp=gaussian_filter(file_var['pa'].data*1, sigma=1.), left_title='1901-1955 Linear Trend', right_title='dsws', out_path=out_path, pic_name="ERL_figSnew_JJA_BTAL_BTALnEU_dsws_linear_trend.pdf", level=level2, pvalue=None)
