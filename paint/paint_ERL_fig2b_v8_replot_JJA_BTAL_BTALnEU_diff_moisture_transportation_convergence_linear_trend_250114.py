...
class paint_class:
    ...
    def plot_uv_diff_at_select_lev(f0, diff_u, diff_v, diff_vint, lev, plot_name, pvalue):
        ...
        level0 = np.array([-1.2, -1, -0.8, -0.6, -0.4, -0.3, -0.2, -0.1, 0, 0.1, 0.2, 0.3, 0.4, 0.6, 0.8, 1, 1.2])
...
#                                transform=ccrs.PlateCarree()))
        ...
        q  =  ax.quiver(cyclic_lon, f0['lat'].data, cyclic_data_u, cyclic_data_v, 
            ...
            scale_units='xy', scale=18,        # scale是参考矢量，所以取得越大画出来的箭头就越短
            units='xy', width=0.4,              # width控制粗细
            transform=proj,
            color='k', headlength = 5, headaxislength = 4, headwidth = 4, alpha=0.8)

        add_vector_legend(ax=ax, q=q, speed=5e1)
...
#    with concurrent.futures.ProcessPoolExecutor() as executor:
    a1 = calculate_class.cal_jjas_mean(file_path=path0, 
                            file_name='BTAL_moisture_flux_integration_1850_150years_1000to100_v2.nc', 
...
#    # =================== 3. Calculate the ttest =====================================
    ...
    ncfile.to_netcdf("/home/sun/data/download_data/data/analysis_data/analysis_EU_aerosol_climate_effect/BTAL_BTALnEU_difference_moisture_transportation_diff_1901to1955_linear_trend_JJA_v7.nc")
...
#    file0 = xr.open_dataset("/home/sun/data/download_data/data/analysis_data/analysis_EU_aerosol_climate_effect/BTAL_BTALnEU_difference_moisture_transportation_diff_1901to1955_linear_trend.nc")
    ...
    lev0 = 850
    paint_class.plot_uv_diff_at_select_lev(f0=file0, diff_u=gaussian_filter((file0["transport_x_BTAL_diff"].data) * 55 - file0["transport_x_BTALnEU_diff"].data * 55 , sigma=0.8), 
                                            diff_v=gaussian_filter((file0["transport_y_BTAL_diff"].data) * 55 - file0["transport_y_BTALnEU_diff"].data * 55 , sigma=0.8), 
                                            diff_vint=gaussian_filter((file0["mt_vint_BTAL_diff"].data) * 55 - file0["mt_vint_BTALnEU_diff"].data * 55 , sigma=0.8), 
                                            lev=850, 
                                            plot_name='ERL_fig2b_v8_BTAL_BTALnEU_Water_transportation_linear_trend',
