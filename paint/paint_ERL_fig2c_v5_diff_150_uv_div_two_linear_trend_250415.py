...
def paint_jjas_diff(u, v, div, p, out_name, left_title, right_title):
    ...
    level0 = np.array([-5, -4, -3, -2, -1, -0.75, -0.5, -0.25, 0.25, 0.5, 0.75, 1, 2, 3, 4, 5])
    im1  =  ax.contourf(lon, lat, div, levels=level0, cmap='coolwarm', alpha=1, extend='both')
...
def main():
    ...
    paint_jjas_diff(55*gaussian_filter((u_con - u_neu), sigma=1), 55*gaussian_filter((v_con - v_neu), sigma=1), 1e7*gaussian_filter((div_con - div_neu), sigma=1)*55, div_p_con, out_name='ERL_fig2c_v5_CESM_BTAL_BTALnEU_150_uv_div_linear_trend.pdf',  left_title="(c)", right_title="150 hPa")
