...
#    set_cartopy_tick(ax=ax,extent=extent,xticks=np.linspace(70,90,3,dtype=int),yticks=np.linspace(0,40,5,dtype=int),nx=1,ny=1,labelsize=15)
    ...
    # --- Set range ---
    lonmin,lonmax,latmin,latmax  =  55,125,5,35
    extent     =  [lonmin,lonmax,latmin,latmax]
...
def main():
    ...
    plot_diff_rainfall(diff_data=gaussian_filter(ncfile['JJAS_trend'].data * 10, sigma=0.5), left_title='(c)', right_title='CESM_ALL (JJAS)', out_path=out_path, pic_name="ERL_fig1c_v2_CESM_prect_BTAL_JJAS_linear_trend_1901to1955.pdf", level=lev0, p=ncfile['JJAS_p'].data)
