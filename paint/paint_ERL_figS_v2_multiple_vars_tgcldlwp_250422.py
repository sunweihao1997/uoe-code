...
file_var    = xr.open_dataset(file_path + 'cesm_allf_fixEU_tgcldlwp_trend_jja.nc')
#print(psl_btal)
...
def main():
    ...
    level2 = np.array([-20, -10, -6, -5, -4, -3, -2, -1, -0.5, 0.5, 1, 2, 3, 4, 5, 6, 10, 20])
...
#    plot_diff_slp_wind(diff_slp=data_file["psl_btal_btalneu_diff"],    diff_u=data_file["u_btal_btalneu_diff"], diff_v=data_file["v_btal_btalneu_diff"] , left_title='(a)', right_title='BTAL - BTALnEU', out_path=out_path, pic_name="Aerosol_research_ERL_2a_BTAL_BTALnEU.pdf", p=data_file['psl_btal_btalneu_diffp'], level=level2)
    plot_diff_slp_wind(diff_slp=gaussian_filter(file_var['pa'].data*1, sigma=1), left_title='1901-1955 Linear Trend', right_title='tgcldlwp', out_path=out_path, pic_name="ERL_figSnew_JJA_BTAL_BTALnEU_tgcldlwp_linear_trend_v2.pdf", level=level2, pvalue=None)
