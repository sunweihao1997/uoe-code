...
p1 = 1940 ; p2 = 1960
ncfile_p = calculate_student_t_test(psl_btal, psl_btalneu, p1, p2, lat, lon, 'TS_JJA')
...
#    plot_diff_slp_wind(diff_slp=data_file["psl_btal_btalneu_diff"],    diff_u=data_file["u_btal_btalneu_diff"], diff_v=data_file["v_btal_btalneu_diff"] , left_title='(a)', right_title='BTAL - BTALnEU', out_path=out_path, pic_name="Aerosol_research_ERL_2a_BTAL_BTALnEU.pdf", p=data_file['psl_btal_btalneu_diffp'], level=level2)
    plot_diff_slp_wind(diff_slp=55*gaussian_filter((psl_con - psl_neu), sigma=1),left_title='1901-1955 Linear Trend', right_title='CESM_ALL - CESM_noEU', out_path=out_path, pic_name="Aerosol_research_ERL_s5c_BTAL_BTALnEU_TS_linear_trend.pdf", level=level2, pvalue=ncfile_p.data)
