...
p1 = 1901 ; p2 = 1955
u_con, u_p_con = calculate_linear_trend(p1, p2, file1, 'btal_u')
v_con, v_p_con = calculate_linear_trend(p1, p2, file1, 'btal_v')
u_neu, u_p_neu = calculate_linear_trend(p1, p2, file1, 'btalneu_u')
v_neu, v_p_neu = calculate_linear_trend(p1, p2, file1, 'btalneu_v')
...
def paint_jjas_diff(u, v, w, p, out_name, left_title, right_title, tlat):
    ...
    # Tick setting
    # --- Set range ---
    lonmin,lonmax,latmin,latmax  =  0,130,0,70
    ...
    q  =  ax.quiver(lon, tlat, u, v, 
        regrid_shape=11, angles='uv',        # regrid_shape这个参数越小，是两门就越稀疏
...
def main():
    ...
    paint_jjas_diff(gaussian_filter((u_con - u_neu), sigma=1.0) * 55, gaussian_filter((v_con - v_neu), sigma=1.0) * 55, 1e6*gaussian_filter((w_con - w_neu), sigma=0.5) * 56, w_p_con, out_name='ERL_fig2dtest_v4_JJA_CESM_BTAL_BTALnEU_150_divergent_uv_div_linear_trend.pdf',  left_title="(d)", right_title="Divergent Wind", tlat=lat)
