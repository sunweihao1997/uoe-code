'''
2024-7-19
This script is to calculate the linear trend of FSDSCS over Europe area between BTAL and BTALnEU
'''
import xarray as xr
import numpy as np
import os
...
def plot_diff_slp_wind(diff_slp, diff_u, diff_v, left_title, right_title, out_path, pic_name, level, pvalue):
...
#    plot_diff_slp_wind(diff_slp=data_file["psl_btal_btalneu_diff"],    diff_u=data_file["u_btal_btalneu_diff"], diff_v=data_file["v_btal_btalneu_diff"] , left_title='(a)', right_title='BTAL - BTALnEU', out_path=out_path, pic_name="Aerosol_research_ERL_2a_BTAL_BTALnEU.pdf", p=data_file['psl_btal_btalneu_diffp'], level=level2)
    plot_diff_slp_wind(diff_slp=1e1*gaussian_filter((psl_con - psl_neu), sigma=0.5), diff_u=0, diff_v=0,  left_title='1901-1955 Linear Trend', right_title='CESM_ALL - CESM_noEU', out_path=out_path, pic_name="Aerosol_research_ERL_s5c_BTAL_BTALnEU_FSDSCS_linear_trend.pdf", level=level2, pvalue=None)
    plot_diff_slp_wind(diff_slp=1e1*gaussian_filter((psl_con), sigma=0.5), diff_u=0, diff_v=0,  left_title='1901-1955 Linear Trend', right_title='CESM_ALL', out_path=out_path, pic_name="Aerosol_research_ERL_s5a_BTAL_BTALnEU_FSDSCS_linear_trend.pdf", level=level2, pvalue=None)
    plot_diff_slp_wind(diff_slp=1e1*gaussian_filter((psl_neu), sigma=0.5), diff_u=0, diff_v=0,  left_title='1901-1955 Linear Trend', right_title='CESM_noEU', out_path=out_path, pic_name="Aerosol_research_ERL_s5b_BTAL_BTALnEU_FSDSCS_linear_trend.pdf", level=level2, pvalue=None)
