...
psl_btal    = xr.open_dataset(file_path + 'BTAL_FSDSCS_jjas_mean_240721.nc')
psl_btalneu = xr.open_dataset(file_path + 'BTALnEU_FSDSCS_jjas_mean_240721.nc')
#print(psl_btal)
...
def calculate_linear_trend_diff(start, end, input_array1, input_array2, varname):
    '''This function is due to that, the first function is only for the single exp'''
    from scipy.stats import linregress

    time_dim, lat_dim, lon_dim = input_array1.sel(time=slice(start, end))[varname].shape

    trend_data = np.zeros((lat_dim, lon_dim))
    p_data     = np.zeros((lat_dim, lon_dim))

    input_data = (input_array1.sel(time=slice(start, end))[varname].data - input_array2.sel(time=slice(start, end))[varname].data)
    #print(input_data.shape)
...
p1 = 1901 ; p2 = 1956
psl_con, psl_p_con = calculate_linear_trend(p1, p2, psl_btal,    'FSDSCS_JJAS')
psl_neu, psl_p_neu = calculate_linear_trend(p1, p2, psl_btalneu, 'FSDSCS_JJAS')
psl_dif, psl_p_dif = calculate_linear_trend_diff(p1, p2, psl_btal, psl_btalneu, 'FSDSCS_JJAS')

#print(np.average(psl_dif))
#sys.exit()
...
def plot_diff_slp_wind(diff_slp, left_title, right_title, out_path, pic_name, level, pvalue):
    ...
    dot  =  ax.contourf(lon, lat, pvalue, levels=[0., 0.12], colors='none', hatches=['/'])
...
#    plot_diff_slp_wind(diff_slp=data_file["psl_btal_btalneu_diff"],    diff_u=data_file["u_btal_btalneu_diff"], diff_v=data_file["v_btal_btalneu_diff"] , left_title='(a)', right_title='BTAL - BTALnEU', out_path=out_path, pic_name="Aerosol_research_ERL_2a_BTAL_BTALnEU.pdf", p=data_file['psl_btal_btalneu_diffp'], level=level2)
    plot_diff_slp_wind(diff_slp=1e1*gaussian_filter(psl_dif, sigma=0.5),left_title='1901-1955 Linear Trend', right_title='CESM_ALL - CESM_noEU', out_path=out_path, pic_name="ERL_fig4_BTAL_BTALnEU_TS_linear_trend.pdf", level=level2, pvalue=psl_p_dif)
    #plot_diff_slp_wind(diff_slp=1e1*gaussian_filter((psl_con), sigma=0.5),          left_title='1901-1955 Linear Trend', right_title='CESM_ALL', out_path=out_path,             pic_name="ERL_fig4_BTAL_BTALnEU_TS_linear_trend.pdf", level=level2, pvalue=None)
    #plot_diff_slp_wind(diff_slp=1e1*gaussian_filter((psl_neu), sigma=0.5),          left_title='1901-1955 Linear Trend', right_title='CESM_noEU', out_path=out_path,            pic_name="ERL_fig4_BTAL_BTALnEU_TS_linear_trend.pdf", level=level2, pvalue=None)
