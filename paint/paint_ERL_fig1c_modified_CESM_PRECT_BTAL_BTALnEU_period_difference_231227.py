...
2024-7-2 modified:
after talking with M, he advised to change the scale of second period
'''
import xarray as xr
import numpy as np
...
#print(data)
data_p1 = data.sel(time=slice(1901, 1915))
data_p2 = data.sel(time=slice(1946, 1960))
...
prect_BTAL_JJAS_DIFF    = np.average(data_p2['PRECT_JJA_BTAL'].data, axis=0)    - np.average(data_p1['PRECT_JJA_BTAL'].data, axis=0)
prect_BTALnEU_JJAS_DIFF = np.average(data_p2['PRECT_JJA_BTALnEU'].data, axis=0) - np.average(data_p1['PRECT_JJA_BTALnEU'].data, axis=0)
...
#    set_cartopy_tick(ax=ax,extent=extent,xticks=np.linspace(70,90,3,dtype=int),yticks=np.linspace(0,40,5,dtype=int),nx=1,ny=1,labelsize=15)
    ...
    # --- Set range ---
    lonmin,lonmax,latmin,latmax  =  50,140,0,50
    extent     =  [lonmin,lonmax,latmin,latmax]
...
#    # Stippling picture
    ...
    # --- title ---
    ax.set_title("1946-1960", loc='left', fontsize=22.5)
...
for yy in range(len(lat)):
    for xx in range(len(lon)):
        ...
        anomaly_periodC_BTAL    = data_p2['PRECT_JJAS_BTAL'].data[:, yy, xx] - np.average(data['PRECT_JJAS_BTAL'].data[:, yy, xx])
        anomaly_periodC_BTALnEU = data_p2['PRECT_JJAS_BTALnEU'].data[:, yy, xx] - np.average(data['PRECT_JJAS_BTALnEU'].data[:, yy, xx])
...
def main():
    ...
    plot_diff_rainfall(diff_data=prect_BTAL_JJAS_DIFF, left_title='(c)', right_title='CESM_ALL (JJA)', out_path=out_path, pic_name="ERL_fig1c_CESM_prect_BTAL_JJAS_period_difference_1900_1960_231221.pdf", p=p_value_BTAL)
#    plot_diff_rainfall(diff_data=prect_BTALnEU_JJAS_DIFF, left_title='(b)', right_title='CESM_noEU (JJAS)', out_path=out_path, pic_name="Aerosol_research_CESM_prect_BTALnEU_JJAS_period_difference_1900_1960_231221.pdf", p=p_value_BTALnEU)
    plot_diff_rainfall(diff_data=(prect_BTAL_JJAS_DIFF - prect_BTALnEU_JJAS_DIFF), left_title='(d)', right_title='CESM_ALL - CESM_noEU (JJA)', out_path=out_path, pic_name="ERL_fig1d_CESM_prect_BTAL_sub_BTALnEU_JJAS_period_difference_1900_1960_231227.pdf", p=p_value_BTAL_BTALnEU)
