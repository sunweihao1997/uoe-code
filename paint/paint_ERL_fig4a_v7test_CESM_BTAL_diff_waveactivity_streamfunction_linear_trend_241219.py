...
def paint_jjas_diff(sf, u, v, p, pic_name, left_title):
    ...
    # Tick setting
    # extent
    lonmin,lonmax,latmin,latmax  =  0,240,10,80
    extent     =  [lonmin,lonmax,latmin,latmax]

    set_cartopy_tick(ax=ax,extent=extent,xticks=np.linspace(0,360,13,dtype=int),yticks=np.linspace(0,80,9,dtype=int),nx=1,ny=1,labelsize=7)
...
#    sys.exit()
    ...
    paint_jjas_diff(ncfile_psi['psidev'].data*1e-5*55, 55 * (ncfile_fx['Fx'].data) * 1e4, 55 * (ncfile_fy['Fy'].data) * 1e4, None, "ERL_fig4a_v6_CESM_BTAL_wave_activity_linear_trend_300_vector_legend.png", '1901-1955')
    print("Paint Success")
