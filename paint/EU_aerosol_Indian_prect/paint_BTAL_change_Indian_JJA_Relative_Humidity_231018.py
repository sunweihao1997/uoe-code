'''
2023-10-22
This script calculate and paint relative humidity changes between two given periods
'''
import xarray as xr
import numpy as np
import os
...
class calculate_class:
    out_path0 = '/exports/csce/datastore/geos/users/s2618078/data/model_data/RELHUM/'
    member_num = 8
    ...
    ensemble_path0 = '/exports/csce/datastore/geos/users/s2618078/data/model_data/RELHUM/'
    ...
    def cal_jja_mean(file_path, file_name, var_name, exp_name,):
        ...
        ncfile.to_netcdf("/exports/csce/datastore/geos/users/s2618078/data/analysis_data/analysis_EU_aerosol_climate_effect/{}_{}_ensemble_mean_JJA_231023.nc".format(var_name, exp_name), format='NETCDF4')
        ncfile.attrs['script']       =  'paint_BTAL_change_Indian_JJA_Relative_Humidity_231018.py'
...
class paint_class:
    ...
    # !!!!!!!!!! Settings !!!!!!!!!!!!
    # ======== Extent ==========
    lonmin,lonmax,latmin,latmax  =  -10,150,-20,75
    ...
    def plot_uv_diff_at_select_lev_among_exps(diff_u1, diff_v1, diff_u2, diff_v2, diff_slp1, diff_slp2, lev,):
        ...
        im  =  ax.contourf(cyclic_lon, diff_u1['lat'].data, cyclic_data_slp, np.linspace(-35,35,15), cmap='coolwarm', alpha=1, extend='both')
...
def main():
    ...
    #    for i in range(8):
    #        executor.submit(calculate_class.cdo_ensemble, src_path, 'BTAL', 'RELHUM', i)
    #        executor.submit(calculate_class.cdo_ensemble, src_path, 'BTALnEU', 'RELHUM', i)
    #for i in range(8):
    #    calculate_class.cdo_ensemble(src_path, 'BTAL', 'RELHUM', i)
    #    calculate_class.cdo_ensemble(src_path, 'BTALnEU', 'RELHUM', i)
    ...
    # === 3. Calculate ensemble mean among the members output === !! Finished 2023-10-22-11:27!!
    #with concurrent.futures.ThreadPoolExecutor() as executor:
    #    executor.submit(calculate_class.cal_ensemble, 'BTAL', 'RELHUM')
    #    executor.submit(calculate_class.cal_ensemble, 'BTALnEU', 'RELHUM')
    ...
    #with concurrent.futures.ProcessPoolExecutor() as executor:
    #    executor.submit(calculate_class.cal_jja_mean,file_path=analysis_data_path, 
    #                                file_name='BTAL_RELHUM_ensemble_mean_231019.nc', 
    #                                var_name='RELHUM',
    #                                exp_name='BTAL',
    #                                )
    #    executor.submit(calculate_class.cal_jja_mean,file_path=analysis_data_path, 
    #                                file_name='BTALnEU_RELHUM_ensemble_mean_231019.nc', 
    #                                var_name='RELHUM',
    #                                exp_name='BTALnEU',
    #                                )

    # === Step 4 !!Finished 2023-10-23-09:32!!===
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        file_con_rh = xr.open_dataset('/exports/csce/datastore/geos/users/s2618078/data/analysis_data/analysis_EU_aerosol_climate_effect/RELHUM_BTAL_ensemble_mean_JJA_231023.nc')
        file_eu_rh  = xr.open_dataset('/exports/csce/datastore/geos/users/s2618078/data/analysis_data/analysis_EU_aerosol_climate_effect/RELHUM_BTALnEU_ensemble_mean_JJA_231023.nc')
        
        a = executor.submit(calculate_class.cal_two_periods_difference, file_con_rh['RELHUM_JJA'].data, 3)
        b = executor.submit(calculate_class.cal_two_periods_difference, file_eu_rh['RELHUM_JJA'].data, 3)
        con_diff_rh = a.result()
        eu_diff_rh  = b.result()
