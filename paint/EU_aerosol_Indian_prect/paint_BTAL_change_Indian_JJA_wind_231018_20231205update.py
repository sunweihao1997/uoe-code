...
class paint_class:
    ...
    def plot_uv_diff_at_select_lev(diff_u, diff_v, diff_slp, lev, plot_name):
        ...
        # Extract level data
        u          =  diff_u.sel(lev=lev)['jjas_diff'].data ; v          =  diff_v.sel(lev=lev)['jjas_diff'].data
        ...
        cyclic_data_slp, cyclic_lon = add_cyclic_point(diff_slp['jjas_diff'].data, coord=diff_u['lon'].data)
        ...
        set_cartopy_tick(ax=ax,extent=[60, 90, 0, 35],xticks=np.linspace(60,90,4,dtype=int),yticks=np.linspace(0,30,4,dtype=int),nx=1,ny=1,labelsize=10)

        im  =  ax.contourf(cyclic_lon, diff_u['lat'].data, cyclic_data_slp, np.linspace(-40,40,9), cmap='coolwarm', alpha=1, extend='both')
        ...
        units='xy', width=0.75,              # width控制粗细
        transform=proj,
        color='k', headlength = 5, headaxislength = 4, headwidth = 4, alpha=0.8)

        ax.set_title('1901-1920 to 1941-1960',fontsize=17.5)
    ...
    def plot_uv_diff_at_select_lev_among_exps(diff_u1, diff_v1, diff_u2, diff_v2, diff_slp1, diff_slp2, lev,):
        ...
        # Extract level data
        u1          =  diff_u1.sel(lev=lev)['jjas_diff'].data ; v1         =  diff_v1.sel(lev=lev)['jjas_diff'].data
        u2          =  diff_u2.sel(lev=lev)['jjas_diff'].data ; v2         =  diff_v2.sel(lev=lev)['jjas_diff'].data
        slp1        =  diff_slp1['jjas_diff'].data            ; slp2       =  diff_slp2['jjas_diff'].data 
        ...
        #    color='k', headlength = 5, headaxislength = 4, headwidth = 4, alpha=0.8)
#        q  =  ax.quiver(cyclic_lon, diff_u1['lat'].data, cyclic_data_u, cyclic_data_v, 
#        regrid_shape=14, angles='uv',        # regrid_shape这个参数越小，是两门就越稀疏
#        scale_units='xy', scale=0.03,        # scale是参考矢量，所以取得越大画出来的箭头就越短
...
#        color='k', headlength = 5, headaxislength = 4, headwidth = 4, alpha=0.8)
        ...
        plt.savefig('/exports/csce/datastore/geos/users/s2618078/paint/analysis_EU_aerosol_climate_effect/slp/BTAL_BTALnEU_period_diff_JJAS_SLP.pdf', dpi=500)
...
#                                )
    ...
    file_con_u = xr.open_dataset('/exports/csce/datastore/geos/users/s2618078/data/analysis_data/analysis_EU_aerosol_climate_effect/U_BTAL_ensemble_mean_JJAS_231019.nc')
    con_diff_u = calculate_class.cal_two_periods_difference(file_con_u['U_JJAS'].data, 3)

    file_con_v = xr.open_dataset('/exports/csce/datastore/geos/users/s2618078/data/analysis_data/analysis_EU_aerosol_climate_effect/V_BTAL_ensemble_mean_JJAS_231019.nc')
    con_diff_v = calculate_class.cal_two_periods_difference(file_con_v['V_JJAS'].data, 3)

    file_eu_u  = xr.open_dataset('/exports/csce/datastore/geos/users/s2618078/data/analysis_data/analysis_EU_aerosol_climate_effect/U_BTALnEU_ensemble_mean_JJAS_231019.nc')
    eu_diff_u  = calculate_class.cal_two_periods_difference(file_eu_u['U_JJAS'].data, 3)

    file_eu_v  = xr.open_dataset('/exports/csce/datastore/geos/users/s2618078/data/analysis_data/analysis_EU_aerosol_climate_effect/V_BTALnEU_ensemble_mean_JJAS_231019.nc')
    eu_diff_v  = calculate_class.cal_two_periods_difference(file_eu_v['V_JJAS'].data, 3)
    ...
    con_diff_slp = calculate_class.cal_two_periods_difference(file_con_slp['PSL_JJAS'].data, 2)
