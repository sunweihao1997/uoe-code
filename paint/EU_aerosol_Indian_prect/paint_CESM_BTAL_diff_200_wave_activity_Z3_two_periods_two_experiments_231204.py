...
def paint_jjas_diff(u, v, z, p, pic_name):
    ...
    im1  =  ax.contourf(lon, lat, z, levels=np.linspace(-20, 20, 21), cmap='coolwarm', alpha=1, extend='both')
    ...
    q  =  ax.quiver(lon, lat, u, v, 
        ...
        scale_units='xy', scale=0.005,        # scale是参考矢量，所以取得越大画出来的箭头就越短
...
def main():
    ...
    u_diff = (BTAL_Fx2['Fx'] - BTAL_Fx1['Fx'])
    v_diff = (BTAL_Fy2['Fy'] - BTAL_Fy1['Fy'])
    paint_jjas_diff(u_diff, v_diff, z_diff_BTAL, ncfile['p_z'].data, pic_name="EUI_CESM_BTAL_wave_activity_period_diff_200_Z3.pdf")
