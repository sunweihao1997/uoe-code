...
class paint_class:
    ...
    def plot_uv_diff_at_select_lev(f0, diff_u, diff_v, diff_vint, lev, plot_name, pvalue):
        ...
        im  =  ax.contourf(cyclic_lon, f0['lat'].data, cyclic_data_vint * 1e5, np.linspace(-15, 15, 16), cmap='coolwarm', alpha=1, extend='both')
        ...
        ax.set_ylabel('ALL_Forcing - Fix_EU', fontsize=15)

        plt.colorbar(im, orientation='horizontal')

        plt.savefig('/exports/csce/datastore/geos/users/s2618078/paint/analysis_EU_aerosol_climate_effect/moisture_transportation/{}_period_diff_JJA_Indian_UV.pdf'.format(plot_name), dpi=500)
...
#
    ...
    paint_class.plot_uv_diff_at_select_lev(f0=file0, diff_u=(file0_lev["transport_x_BTAL_diff"].data - file0_lev["transport_x_BTALnEU_diff"].data), 
                                            ...
                                            diff_vint=(file0["mt_vint_BTAL_diff"].data - file0["mt_vint_BTALnEU_diff"].data), 
                                            lev=850, 
                                            plot_name='Water_transportation',
