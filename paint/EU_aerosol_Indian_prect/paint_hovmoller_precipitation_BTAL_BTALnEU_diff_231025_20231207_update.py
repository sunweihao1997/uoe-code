...
def paint_rain_evolution_diff(ncfile, varname, name, title0):
    ...
    im  =  ax.contourf(np.linspace(1, 12, 12), ncfile1['lat'].data, np.swapaxes(np.nanmean(ncfile1[varname].data, axis=2), 0, 1) * 86400 * 1000, levels, cmap='bwr_r', alpha=1, extend='both')

    ax.set_xticks(np.linspace(1, 12, 12))
    ax.set_yticks(np.linspace(-10, 40, 6))
    ax.set_ylim((5, 40))

    ax.set_ylabel(title0, fontsize=12.5)
...
def main():
    ...
    # interpolate mask file
    mask_file = mask_file.interp(latitude=prect1.lat.data, longitude=prect1.lon.data)

    prect1['PRECC'].data[:, mask_file['lsm'].data[0] < 0.1] = np.nan
    prect1['PRECL'].data[:, mask_file['lsm'].data[0] < 0.1] = np.nan
    prect0['PRECC'].data[:, mask_file['lsm'].data[0] < 0.1] = np.nan
    prect0['PRECL'].data[:, mask_file['lsm'].data[0] < 0.1] = np.nan
...
#    a = calculation.cal_period_average(data=prect0['PRECC'], dim_num=2, start=1900, end=1920, varname='PRECC')
    ...
    paint_rain_evolution_diff(ncfile, "diff_con_neu", "BTAL_BTALnEU_diff.pdf", 'BTAL - BTALnEU')
    paint_rain_evolution_diff(ncfile, "diff_p1p2_con", "BTAL.pdf", 'BTAL')
    paint_rain_evolution_diff(ncfile, "diff_p1p2_neu", "BTALnEU.pdf", 'BTALnEU')
