...
src_file  =  xr.open_dataset('/exports/csce/datastore/geos/users/s2618078/data/analysis_data/analysis_EU_aerosol_climate_effect/BTAL_TS_ensemble_mean_231017.nc')
...
class cal_function:
    ...
    def write_file(data):
        ...
        ncfile.attrs['description']  =  'Created on 2023-10-16. This file is the JJA ensemble mean among the 8 member in the BTAL emission experiments. The variables are TS.'
        ncfile.to_netcdf("/exports/csce/datastore/geos/users/s2618078/data/analysis_data/BTAL_sulfate_column_burden_jja_mean_231017.nc", format='NETCDF4')

    out_path = '/exports/csce/datastore/geos/users/s2618078/data/model_data/TS/'
    member_num = 8
    def cdo_ensemble(path, exp_name, var_name, member_num):
        ...
        for i in range(member_num):
            ...
            os.system('cdo cat ' + path_src + '*nc ' + cal_function.out_path + 'BTAL_TS_1850_150years_member_' + str(i + 1) + '.nc')
    ...
    def cal_ensemble(var_name):
        ...
        for i in range(cal_function.member_num):
            f0 = xr.open_dataset(cal_function.out_path + 'BTAL_TS_1850_150years_member_' + str(i + 1) +'.nc')
        ...
        ncfile.attrs['description']  =  'Created on 2023-10-17. This file is the ensemble mean among the 8 member in the BTAL emission experiments. The variable is surface temperature.'
        ncfile.to_netcdf("/exports/csce/datastore/geos/users/s2618078/data/analysis_data/BTAL_TS_ensemble_mean_231017.nc", format='NETCDF4')

    file_name  = '/exports/csce/datastore/geos/users/s2618078/data/analysis_data/BTAL_TS_ensemble_mean_231017.nc'
    def cal_jja_mean():
        ...
        ncfile.to_netcdf("/exports/csce/datastore/geos/users/s2618078/data/analysis_data/BTAL_TS_ensemble_mean_JJA_231017.nc", format='NETCDF4')
...
class plot_function:
    ...
    # ======== Set Extent ==========
    #lonmin,lonmax,latmin,latmax  =  30,135,-20,60
...
#    newcmp.set_under('white')
    ...
    def paint_jja_diff(data):
        ...
        set_cartopy_tick(ax=ax,extent=plot_function.extent,xticks=np.linspace(50,110,4,dtype=int),yticks=np.linspace(-10,40,6,dtype=int),nx=1,ny=1,labelsize=12)
        ...
        ax.set_ylabel("ALL_FORCING", fontsize=15)
        ...
        plt.savefig('/exports/csce/datastore/geos/users/s2618078/paint/analysis_EU_aerosol_climate_effect/TS/BTAL_diff_JJA_Indian_TS.pdf')

def main():
    # === 1. CDO emsemble members files === !!Finished 2023-10-17-15:58!!
    #cal_function.cdo_ensemble('/exports/csce/datastore/geos/groups/aerosol/sabineCESM/BTAL/', 'BTAL', 'TS', 8)

    # === 2. Calculate ensemble mean among the members output === !! Finished 2023-10-17-16:09!!
    #cal_function.cal_ensemble('TS')

    # === 3. Calculate JJA mean === !!Finished 2023-10-17-16:10!! /exports/csce/datastore/geos/users/s2618078/data/analysis_data/BTAL_TS_ensemble_mean_JJA_231017.nc
    #cal_function.cal_jja_mean()

    # === 4. Calculate difference between two given period ===
#    file0 = xr.open_dataset('/exports/csce/datastore/geos/users/s2618078/data/analysis_data/BTAL_TS_ensemble_mean_JJA_231017.nc')
...
#    ncfile.attrs['description']  =  'Created on 2023-10-17. This file is the difference for pBTAL_TS_ensemble_mean_JJA_231017.nceriod (1900-1920) and (1940-1960) JJA ensemble mean among the 8 member in the BTAL emission experiments. The variables are TS_JJA.'
#    ncfile.to_netcdf("/exports/csce/datastore/geos/users/s2618078/data/analysis_data/BTAL_surface_temperature_jja_mean_diff_1900_1960_231017.nc", format='NETCDF4')
#    ncfile.attrs['script']       =  'paint_change_Indian_surface_temperature_between_period_231017.py'
    # === 5 Step Succeed ===

    # === 6. painting ===
    file1 = xr.open_dataset('/exports/csce/datastore/geos/users/s2618078/data/analysis_data/analysis_EU_aerosol_climate_effect/BTAL_surface_temperature_jja_mean_diff_1900_1960_231017.nc')
