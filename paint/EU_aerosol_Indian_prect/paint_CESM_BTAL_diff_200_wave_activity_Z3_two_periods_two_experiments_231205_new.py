...
def paint_jjas_diff(u, v, z, p, pic_name):
    ...
    im1  =  ax.contourf(lon, lat, z, levels=np.linspace(-15, 15, 25), cmap='coolwarm', alpha=1, extend='both')
    ...
    q  =  ax.quiver(lon, lat, u, v, 
        ...
        scale_units='xy', scale=0.0015,        # scale是参考矢量，所以取得越大画出来的箭头就越短
    ...
    plt.savefig('/exports/csce/datastore/geos/users/s2618078/paint/analysis_EU_aerosol_climate_effect/circulation/{}'.format(pic_name))
...
def main():
    ...
    Fx = xr.open_dataset(wave_path + "BTAL_BTALnEU_diff_Z3_for_TN2001-Fx.monthly.period2.nc").sel(level=200)
    Fy = xr.open_dataset(wave_path + "BTAL_BTALnEU_diff_Z3_for_TN2001-Fy.monthly.period2.nc").sel(level=200)
    ...
    u_diff = Fx['Fx']
    v_diff = Fy['Fy']
    paint_jjas_diff(u_diff, v_diff, z_diff, ncfile['p_z'].data, pic_name="EUI_CESM_BTAL_wave_activity_period_diff_200_Z3_new_calculation_method.pdf")
