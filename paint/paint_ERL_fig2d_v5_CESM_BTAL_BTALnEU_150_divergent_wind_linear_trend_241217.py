...
file_src2= 'EUI_CESM_BTAL_BTALnEU_high_level_but_single_level_200_ensemble_mean_UVWZ_DIV_and_ttest_1945_1960_JJA.nc'
file_src = 'Aerosol_Research_CESM_200hpa_divergent_wind_divergence_BTAL_BTALnEU_JJA.nc'
...
def paint_jjas_diff(u, v, w, p, out_name, left_title, right_title, tlat):
    ...
    add_vector_legend(ax=ax, q=q, speed=.25)
...
def main():
    ...
    paint_jjas_diff(1e2*gaussian_filter((u_con - u_neu), sigma=0.5), 1e2*gaussian_filter((v_con - v_neu), sigma=0.5), 1e8*gaussian_filter((w_con - w_neu), sigma=0.5), w_p_con, out_name='ERL_fig2d_v4_CESM_BTAL_BTALnEU_150_divergent_uv_div_linear_trend.pdf',  left_title="(d)", right_title="Divergent Wind", tlat=lat)
