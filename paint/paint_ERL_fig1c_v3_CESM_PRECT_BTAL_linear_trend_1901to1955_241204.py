...
#
#        fiter_jja = scipy.signal.filtfilt(b, a, data_01to55['PRECT_JJAS_BTAL'].data[:, i, j], axis=0)
#        print(fiter_jja.shape)
        ...
        slope, intercept = np.linalg.lstsq(A, data_01to55['PRECT_JJA'].data[:, i, j], rcond=None)[0]
...
ncfile.to_netcdf(out_path + 'Aerosol_Research_CESM_BTAL_PRECT_JJA_JJAS_linear_trend_1901to1955_test.nc')
...
def main():
    ...
    plot_diff_rainfall(diff_data=ncfile['JJAS_trend'].data * 55 * 86400000, left_title='(c)', right_title='CESM_ALL (JJA)', out_path=out_path, pic_name="ERL_fig1c_v3_CESM_prect_BTAL_JJAS_linear_trend_1901to1955_new.pdf", level=lev0, p=None)
