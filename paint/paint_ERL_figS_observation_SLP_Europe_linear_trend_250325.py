...
psl_btal    = xr.open_dataset(file_path + 'hadslp_trend_jja.nc')
psl_btalneu = xr.open_dataset(file_path + 'noEU_SLP_jja_mean_241211.nc')

#print(psl_btal) ; sys.exit()
#psl_btal = psl_btal.interpolate_na(dim="lon", method="polynomial")  # 先沿 x 插值
#psl_btal = psl_btal.interpolate_na(dim="lat", method="polynomial")  # 再沿 y 插值

psl_btal = psl_btal.interp(lon=psl_btalneu.lon.data, lat=psl_btalneu.lat.data)
#print(psl_btal[
#    'pa'
#].data[10, :])
#sys.exit()
...
#    # Stippling picture
    ...
    # ========= add colorbar =================
    cb  =  fig.colorbar(im, shrink=0.85, pad=0.1, orientation='horizontal', ticks=levels.astype(int))
...
def main():
    ...
    level2    =  np.array([-80, -50, -40, -30, -25, -20, -15, -10, -5, 5, 10, 15, 20, 25, 30, 40, 50, 80])
...
#    plot_diff_slp_wind(diff_slp=data_file["psl_btal_btalneu_diff"],    diff_u=data_file["u_btal_btalneu_diff"], diff_v=data_file["v_btal_btalneu_diff"] , left_title='(a)', right_title='BTAL - BTALnEU', out_path=out_path, pic_name="Aerosol_research_ERL_2a_BTAL_BTALnEU.pdf", p=data_file['psl_btal_btalneu_diffp'], level=level2)
    plot_diff_slp_wind(diff_slp=gaussian_filter((psl_btal['pa'].data), sigma=1)*100,left_title='1901-1955 Linear Trend', right_title='CESM_ALL - CESM_noEU', out_path=out_path, pic_name="ERL_figS_JJA_Observation_PSL_linear_trend.pdf", level=level2, pvalue=None)
