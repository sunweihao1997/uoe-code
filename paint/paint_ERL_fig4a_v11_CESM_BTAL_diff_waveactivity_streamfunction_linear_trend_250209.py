...
def paint_jjas_diff(sf, u, v, p, pic_name, left_title):
    ...
    level0 = np.array([-2.5, -2, -1.5, -1, -0.8, -0.6, -0.4, -0.2, -0.1, -0.05, 0.05, 0.1, 0.2, 0.4, 0.6, 0.8, 1, 1.5, 2, 2.5]) * 10
    norm = BoundaryNorm(level0, ncolors=256, clip=True)
    #im1  =  ax.contour(lon, lat, sf_d, levels=level0, colors='k', alpha=1, linewidths=0.75)
...
#    ax.clabel(im2, fontsize=5, inline=True)
    ...
    add_vector_legend(ax=ax, q=q, speed=20, fontsize=6, location=(0.8, 0), length=0.225, quiver_x=0.9)
...
#    sys.exit()
    ...
    paint_jjas_diff(ncfile_psi['psidev'].data*1e-5*55, 55 * (ncfile_fx['Fx'].data) * 1e4, 55 * (ncfile_fy['Fy'].data) * 1e4, None, "ERL_fig4a_v11_CESM_BTAL_wave_activity_linear_trend_300_no_vector_legend.pdf", '1901-1955')
    print("Paint Success")
