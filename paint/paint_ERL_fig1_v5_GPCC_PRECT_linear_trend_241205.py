...
#    newcmp = ListedColormap(newcolors)
    ...
    im  =  ax.contourf(lon, lat, diff, levels=level, cmap=newcmp, alpha=1, extend='both')
...
#                                transform=ccrs.PlateCarree()))
    ...
    cb.set_ticks(np.array([-2, -1.5, -1, -0.5, -0.1, 0.1, 0.5, 1, 1.5, 2]))  # 自定义刻度位置
    cb.set_ticklabels(np.array([-2, -1.5, -1, -0.5, -0.1, 0.1, 0.5, 1, 1.5, 2]))  # 自定义标签
...
#    pvalue = xr.open_dataset("/home/sun/data/download_data/data/analysis_data/Aerosol_research_GPCC_JJAS_periods_pvalue.nc")
    lev0 = np.array([-2, -1.75, -1.5, -1.25, -1, -0.75, -0.5, -0.25, -0.1, 0.1, 0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2,])
    paint_trend(lat=lat, lon=lon, diff=f0['JJA_trend'].data * 55, level=lev0, p=f0['JJA_p'].data, title_name='1901-1955', pic_path='/home/sun/paint/ERL/', pic_name="ERL_fig1a_v5_Aerosol_Research_GPCC_PRECT_JJA_period_linear_trend_1901to1955.pdf")
