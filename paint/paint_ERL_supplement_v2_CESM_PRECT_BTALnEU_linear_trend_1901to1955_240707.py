...
for i in range(len(lat)):
    for j in range(len(lon)):
        slope, intercept, r_value, p_value, std_err = stats.linregress(np.linspace(1, 55, 55), data_01to55['PRECT_JJA_BTALnEU'].data[:, i, j])
        jja_trend[i, j]  = slope
        jja_p[i, j]      = p_value

        slope, intercept, r_value, p_value, std_err = stats.linregress(np.linspace(1, 55, 55), data_01to55['PRECT_JJAS_BTALnEU'].data[:, i, j])
...
ncfile.attrs['script'] = 'paint_ERL_supplement_v2_CESM_PRECT_BTALnEU_linear_trend_1901to1955_240707.py on UOE'
#
out_path = '/exports/csce/datastore/geos/users/s2618078/data/analysis_data/'
ncfile.to_netcdf(out_path + 'Aerosol_Research_CESM_BTALnEU_PRECT_JJA_JJAS_linear_trend_1901to1955.nc')
...
def main():
    ...
    plot_diff_rainfall(diff_data=ncfile['JJAS_trend'].data * 10, left_title='Fix_EUemission', right_title='JJAS', out_path=out_path, pic_name="ERL_supplement_v2_CESM_prect_BTALnEU_JJAS_linear_trend_1901to1955.pdf", level=lev0, p=ncfile['JJAS_p'].data)
