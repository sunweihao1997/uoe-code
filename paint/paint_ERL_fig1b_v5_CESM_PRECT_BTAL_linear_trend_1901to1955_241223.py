...
for i in range(len(lat)):
    for j in range(len(lon)):
        ...
        years_x = np.linspace(start0, end0, end0 - start0 + 1)
        data_01to60 = data.sel(time=slice(start0, end0))
        slope0, intercept, r_value, p_value, std_err = stats.linregress(np.linspace(start0, end0, end0 - start0 + 1), data_01to60['PRECT_JJA'].data[:, i, j])
        jja_p[i, j]      = p_value
...
#    # Stippling picture
    sp  =  ax.contourf(lon, lat, p, levels=[0., 0.10], colors='none', hatches=['.'])
...
def main():
    ...
    lev0 = np.array([-1.5, -1, -0.8, -0.6, -0.4, -0.3, -0.2, -0.1, -0.05, 0.05, 0.1, 0.2, 0.3, 0.4, 0.6, 0.8, 1, 1.5,])
    ...
    plot_diff_rainfall(diff_data=gaussian_filter(ncfile['JJA_trend'].data * 55 * 86400000, sigma=0.9), left_title='(c)', right_title='CESM_ALL (JJA)', out_path=out_path, pic_name="ERL_fig1b_v5_CESM_prect_BTAL_JJA_linear_trend_1901to1955.pdf", level=lev0, p=ncfile['JJA_p'].data)
