...
v7 modified:
change student t test to mk-trend test

v8 modified:
add arrow regarding the transportation
'''
import xarray as xr
import numpy as np
import os
...
class paint_class:
    ...
    def plot_uv_diff_at_select_lev(f0, diff_u, diff_v, diff_vint, lev, plot_name, pvalue):
        ...
        level0 = np.array([-1.5, -1, -0.8, -0.6, -0.4, -0.3, -0.2, -0.1,  0.1, 0.2, 0.3, 0.4, 0.6, 0.8, 1, 1.2])
        norm = BoundaryNorm(level0, ncolors=256, clip=True)
        im  =  ax.contourf(cyclic_lon, f0['lat'].data, cyclic_data_vint * 1e4, level0, cmap='coolwarm', alpha=1, extend='both', norm=norm)
...
#                                transform=ccrs.PlateCarree()))
        ...
        dot  =  ax.contourf(f0['lon'].data, f0['lat'].data, pvalue, levels=[0., 0.15], colors='none', hatches=['.'])
        ...
        # Vector Map
        q  =  ax.quiver(cyclic_lon, f0['lat'].data, cyclic_data_u, cyclic_data_v, 
            regrid_shape=12.5, angles='uv',        # regrid_shape这个参数越小，是两门就越稀疏
            scale_units='xy', scale=22,        # scale是参考矢量，所以取得越大画出来的箭头就越短
            units='xy', width=0.4,              # width控制粗细
            transform=proj,
            color='k', headlength = 5, headaxislength = 4, headwidth = 4, alpha=0.8)

        add_vector_legend(ax=ax, q=q, speed=50)
...
#    with concurrent.futures.ProcessPoolExecutor() as executor:
    a1 = calculate_class.cal_jjas_mean(file_path=path0, 
                            file_name='BTAL_moisture_flux_integration_1850_150years_1000to300.nc', 
                            var_name='mt_u_integrate_BTAL',
                            exp_name='BTAL',
                            dim_num=2,
                            )
    a2 = calculate_class.cal_jjas_mean(file_path=path0, 
                            file_name='BTAL_moisture_flux_integration_1850_150years_1000to300.nc', 
                            var_name='mt_u_integrate_BTALnEU',
                            exp_name='BTALnEU',
                            dim_num=2,
                            )
    a3 = calculate_class.cal_jjas_mean(file_path=path0, 
                            file_name='BTAL_moisture_flux_integration_1850_150years_1000to300.nc', 
                            var_name='mt_v_integrate_BTAL',
                            exp_name='BTAL',
                            dim_num=2,
                            )
    a4 = calculate_class.cal_jjas_mean(file_path=path0, 
                            file_name='BTAL_moisture_flux_integration_1850_150years_1000to300.nc', 
                            var_name='mt_v_integrate_BTALnEU',
                            exp_name='BTALnEU',
                            dim_num=2,
                            )
    ...
    merge_array = xr.merge([a1, a2, a3, a4, a5, a6])
    print('test done!')
#    sys.exit()
#    print(merge_array)
#    sys.exit()
...
#    print(calculate_class.cal_two_periods_trend(merge_array['mt_vint_BTAL_JJAS'].data - merge_array['mt_vint_BTALnEU_JJAS'].data, 50, 105)[1].shape)
    ...
    ncfile  =  xr.Dataset(
    {
        "transport_x_BTAL_diff": (["lat", "lon"],    calculate_class.cal_two_periods_trend(merge_array['mt_u_integrate_BTAL_JJAS'].data, p1, p2)[0]),
        "transport_y_BTAL_diff": (["lat", "lon"],    calculate_class.cal_two_periods_trend(merge_array['mt_v_integrate_BTAL_JJAS'].data, p1, p2)[0]),
        "transport_x_BTALnEU_diff": (["lat", "lon"], calculate_class.cal_two_periods_trend(merge_array['mt_u_integrate_BTALnEU_JJAS'].data, p1, p2)[0]),
        "transport_y_BTALnEU_diff": (["lat", "lon"], calculate_class.cal_two_periods_trend(merge_array['mt_v_integrate_BTALnEU_JJAS'].data, p1, p2)[0]),
...
#    # =================== 3. Calculate the ttest =====================================
    ncfile.attrs['description']  =  "Created on 2025-1-13 by /home/sun/uoe-code/paint/paint_ERL_fig2b_v7_replot_JJA_BTAL_BTALnEU_diff_moisture_transportation_convergence_linear_trend_241217.py. This file saves the moisture transportation."
    ncfile.to_netcdf("/home/sun/data/download_data/data/analysis_data/analysis_EU_aerosol_climate_effect/BTAL_BTALnEU_difference_moisture_transportation_diff_1901to1955_linear_trend_JJA_1000to300_integral.nc")
...
#    file0 = xr.open_dataset("/home/sun/data/download_data/data/analysis_data/analysis_EU_aerosol_climate_effect/BTAL_BTALnEU_difference_moisture_transportation_diff_1901to1955_linear_trend.nc")
    ...
    lev0 = 850
    paint_class.plot_uv_diff_at_select_lev(f0=file0, diff_u=gaussian_filter((file0["transport_x_BTAL_diff"].data) * 55 - file0["transport_x_BTALnEU_diff"].data * 55 , sigma=1), 
                                            diff_v=gaussian_filter((file0["transport_y_BTAL_diff"].data) * 55 - file0["transport_y_BTALnEU_diff"].data * 55 , sigma=1), 
                                            diff_vint=gaussian_filter((file0["mt_vint_BTAL_diff"].data) * 55 - file0["mt_vint_BTALnEU_diff"].data * 55 , sigma=1), 
                                            lev=850, 
                                            plot_name='ERL_fig2b_v9_BTAL_BTALnEU_Water_transportation_linear_trend_1000to300',
