...
def main():
    ...
    level2 = np.array([-4, -3, -2, -1, -0.8, -0.6, -0.4, -0.2, 0.2, 0.4, 0.6, 0.8, 1, 2, 3, 4])*1e1
...
#    plot_diff_slp_wind(diff_slp=data_file["psl_btal_btalneu_diff"],    diff_u=data_file["u_btal_btalneu_diff"], diff_v=data_file["v_btal_btalneu_diff"] , left_title='(a)', right_title='BTAL - BTALnEU', out_path=out_path, pic_name="Aerosol_research_ERL_2a_BTAL_BTALnEU.pdf", p=data_file['psl_btal_btalneu_diffp'], level=level2)
    plot_diff_slp_wind(diff_slp=gaussian_filter(file_var['pa'].data*1e1, sigma=1.), left_title='1901-1955 Linear Trend', right_title='dswsc', out_path=out_path, pic_name="ERL_figSnew_JJA_BTAL_BTALnEU_dswsc_linear_trend_v2.pdf", level=level2, pvalue=None)
