...
#        #slope, intercept, r_value, p_value, std_err = stats.linregress(np.linspace(start0, end0, end0 - start0 + 1), data_01to55['PRECT_JJA'].data[:, i, j] - data1_01to55['PRECT_JJA'].data[:, i, j])
#        #jja_trend[i, j]  = slope
#        #jja_p[i, j]      = p_value
#
#        A = np.vstack([years_x, np.ones_like(years_x)]).T
#        slope, intercept = np.linalg.lstsq(A, data_01to55['PRECT_JJA'].data[:, i, j] - data1_01to55['PRECT_JJA'].data[:, i, j], rcond=None)[0] ; print('Ho ye')
#        jja_trend[i, j]  = slope
#
#        # t_stat, p_value = ttest_ind(group1, group2)
#
#        t_stat, p_value = ttest_ind(data_01to55['PRECT_JJA'].data[45:, i, j], data1_01to55['PRECT_JJA'].data[45:, i, j])
#
#        jja_p[i, j] = p_value
#
...
#        # t_stat, p_value = ttest_ind(group1, group2)
#
#        t_stat, p_value = ttest_ind(data_01to55['PRECT_JJA'].data[:, i, j], data1_01to55['PRECT_JJA'].data[:, i, j])
#
#        jja_p[i, j] = p_value
...
#    # Stippling picture
    sp  =  ax.contourf(lon, lat, p, levels=[0., 0.125], colors='none', hatches=['.'])
    ...
    cb.set_ticklabels(level, fontsize=20, rotation=45)  # 自定义标签
...
def main():
    ...
    plot_diff_rainfall(diff_data=gaussian_filter(ncfile['JJA_trend'].data * 55 * 86400000, sigma=0.9), left_title='(c)', right_title='CESM_ALL (JJA)', out_path=out_path, pic_name="ERL_fig1d_v5_CESM_prect_BTAL_minus_BTALnEU_JJA_linear_trend_1901to1955.pdf", level=lev0, p=ncfile['JJA_p'].data)
