...
#print(psl_btal)
print(np.nanmean(psl_btal['PSL_JJAS'].data))
sys.exit()
...
p1 = 1945 ; p2 = 1960
ncfile_p = calculate_student_t_test(psl_btal, psl_btalneu, p1, p2, lat, lon, 'PSL_JJAS')
...
#    plot_diff_slp_wind(diff_slp=data_file["psl_btal_btalneu_diff"],    diff_u=data_file["u_btal_btalneu_diff"], diff_v=data_file["v_btal_btalneu_diff"] , left_title='(a)', right_title='BTAL - BTALnEU', out_path=out_path, pic_name="Aerosol_research_ERL_2a_BTAL_BTALnEU.pdf", p=data_file['psl_btal_btalneu_diffp'], level=level2)
    plot_diff_slp_wind(diff_slp=1e1*gaussian_filter((psl_con - psl_neu), sigma=0.35),left_title='1901-1955 Linear Trend', right_title='CESM_ALL - CESM_noEU', out_path=out_path, pic_name="ERL_fig3d_v5_JJAS_BTAL_BTALnEU_PSL_linear_trend.pdf", level=level2, pvalue=ncfile_p.data)
