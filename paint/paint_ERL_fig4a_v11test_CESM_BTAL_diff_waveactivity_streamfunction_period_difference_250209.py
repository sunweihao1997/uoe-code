...
def paint_jjas_diff(sf, u, v, p, pic_name, left_title):
    ...
    level0 = np.array([-2.5, -2, -1.5, -1, -0.8, -0.6, -0.4, -0.2, -0.1, 0.1, 0.2, 0.4, 0.6, 0.8, 1, 1.5, 2, 2.5]) * 10
    norm = BoundaryNorm(level0, ncolors=256, clip=True)
    im1  =  ax.contourf(lon, lat, sf_d, levels=level0, cmap='coolwarm', alpha=1, extend='both', norm=norm)
...
#    ax.clabel(im2, fontsize=5, inline=True)
    ...
    q  =  ax.quiver(lon, lat, u, v, 
        ...
        scale_units='xy', scale=2.75     ,        # scale是参考矢量，所以取得越大画出来的箭头就越短
...
def main():
    ...
    ncfile_fx   =  xr.open_dataset('/home/sun/data/download_data/data/wave_activity_corrected/BTAL_BTALnEU_diff_Z3_for_TN2001-Fx.monthly.1901_1915_1945_1960_perioddiff.nc').sel(level=300)
    ncfile_fy   =  xr.open_dataset('/home/sun/data/download_data/data/wave_activity_corrected/BTAL_BTALnEU_diff_Z3_for_TN2001-Fy.monthly.1901_1915_1945_1960_perioddiff.nc').sel(level=300)
    ncfile_psi  =  xr.open_dataset('/home/sun/data/download_data/data/wave_activity_corrected/BTAL_BTALnEU_diff_Z3_for_psidev.monthly.1901_1915_1945_1960_perioddiff.nc').sel(level=300)
...
#    sys.exit()
    ...
    paint_jjas_diff(ncfile_psi['psidev'].data*1e-7*55, 55 * (ncfile_fx['Fx'].data) * 1e1, 55 * (ncfile_fy['Fy'].data) * 1e1, None, "ERL_fig4a_v11test_CESM_BTAL_wave_activity_period_diff_300_vector_legend.pdf", '1901-1955')
    print("Paint Success")
