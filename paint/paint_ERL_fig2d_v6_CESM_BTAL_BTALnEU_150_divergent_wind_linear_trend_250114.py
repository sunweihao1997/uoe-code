...
def paint_jjas_diff(u, v, w, p, out_name, left_title, right_title, tlat):
    ...
    im1  =  ax.contourf(lon, tlat, w, levels=level0, cmap='coolwarm', alpha=1, extend='both', norm=norm)
    ...
    q  =  ax.quiver(lon, tlat, u, v, 
        regrid_shape=1, angles='uv',        # regrid_shape这个参数越小，是两门就越稀疏
        scale_units='xy', scale=.03,        # scale是参考矢量，所以取得越大画出来的箭头就越短
...
def main():
    ...
    paint_jjas_diff(gaussian_filter((u_con - u_neu), sigma=1.0) * 55, gaussian_filter((v_con - v_neu), sigma=1.0) * 55, 1e6*gaussian_filter((w_con - w_neu), sigma=0.8) * 56, w_p_con, out_name='ERL_fig2c_v6_JJA_CESM_BTAL_BTALnEU_150_divergent_uv_div_linear_trend.pdf',  left_title="(d)", right_title="Divergent Wind", tlat=lat)
