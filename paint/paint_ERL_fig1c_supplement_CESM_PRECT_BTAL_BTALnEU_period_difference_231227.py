...
2024-1-20 modified:
revise the name and number of the picture
...
def plot_diff_rainfall(diff_data, left_title, right_title, out_path, pic_name, p):
   ...
   # --- Set range ---
    lonmin,lonmax,latmin,latmax  =  65,135,0,40
    extent     =  [lonmin,lonmax,latmin,latmax]

    # --- Tick setting ---
    set_cartopy_tick(ax=ax,extent=extent,xticks=np.linspace(70,130,7,dtype=int),yticks=np.linspace(0,40,5,dtype=int),nx=1,ny=1,labelsize=15)
...
def main():
    ...
    plot_diff_rainfall(diff_data=prect_BTAL_JJAS_DIFF, left_title='(c)', right_title='CESM_ALL', out_path=out_path, pic_name="ERL_fig1c_supplement_CESM_prect_BTAL_JJAS_period_difference_1900_1960_231221.pdf", p=p_value_BTAL)
    plot_diff_rainfall(diff_data=prect_BTALnEU_JJAS_DIFF, left_title='(b)', right_title='CESM_noEU', out_path=out_path, pic_name="ERL_figS2b_Aerosol_research_CESM_prect_BTALnEU_JJAS_period_difference_1900_1960_231221.pdf", p=p_value_BTALnEU)
    plot_diff_rainfall(diff_data=(prect_BTAL_JJAS_DIFF - prect_BTALnEU_JJAS_DIFF), left_title='(d)', right_title='CESM_ALL - CESMnoEU', out_path=out_path, pic_name="ERL_fig1d_supplement_CESM_prect_BTAL_sub_BTALnEU_JJAS_period_difference_1900_1960_231227.pdf", p=p_value_BTAL_BTALnEU)
