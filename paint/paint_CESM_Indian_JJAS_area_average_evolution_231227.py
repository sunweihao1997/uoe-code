...
def plot_area_average_evolution(f0, varname, extent):
    ...
    area_mean = np.zeros((8, years))

    # 3. Calculate for each year
    for mm in range(8):
        for yy in range(years):
            area_mean[mm, yy] = np.average(f0_area[varname+str(mm+1)].data[yy])
    
    area_mean_member = np.average(area_mean, axis=0)
    ...
    area_mean_filter = scipy.signal.filtfilt(b, a, area_mean_member, axis=0)
...
#    print('The length of area_mean is {}'.format(len(area_mean_smooth)))
    ...
    #ax.plot(year, area_mean_member - np.average(area_mean_member), color='grey', linewidth=1.5)
...
def main():
    f1 = xr.open_dataset('/exports/csce/datastore/geos/users/s2618078/data/CESM_BTAL_esemble_JJAS_precipitation.nc').sel(time=slice(1900, 2000))
    plot_area_average_evolution(f1, 'JJAS_prect_', [18, 28, 70, 87])
    #print(JJAS_mean)
